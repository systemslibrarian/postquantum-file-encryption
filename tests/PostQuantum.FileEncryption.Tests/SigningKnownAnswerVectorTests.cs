using System.Text;
using PostQuantum.FileEncryption.Signing;
using PostQuantum.FileEncryption.Signing.Internal;
using Xunit;

namespace PostQuantum.FileEncryption.Tests;

/// <summary>
/// Pinned known-answer vector for the detached-signature sidecar (v1). The signature was
/// produced by an earlier build and must keep verifying against the pinned public key and
/// message. ML-DSA signing is hedged (randomized), so the vector is verify-only — it pins
/// the sidecar layout, the domain-separation context, and the SHA-512 pre-hash. If a change
/// breaks it, that is a deliberate, breaking change — bump the signature FormatVersion and
/// regenerate the vector on purpose.
/// </summary>
public sealed class SigningKnownAnswerVectorTests
{
    private static readonly byte[] Message =
        Encoding.UTF8.GetBytes("PostQuantum.FileEncryption.Signing known-answer vector v1.");

    // Ed25519-pk(32) ‖ ML-DSA-65-pk(1952), base64.
    private const string PublicKeyVector =
        "";

    // PQSG sidecar v1: Magic(4) ‖ Version(1) ‖ AlgId(1) ‖ Ed25519-sig(64) ‖ ML-DSA-65-sig(3309), base64.
    private const string SignatureVector =
        "";

    [Fact]
    public void Pinned_signature_verifies_against_pinned_public_key()
    {
        var publicKey = PqSigningPublicKey.Import(Convert.FromBase64String(PublicKeyVector));
        byte[] signature = Convert.FromBase64String(SignatureVector);
        Assert.Equal(HybridSigning.SignatureLength, signature.Length);

        new PqVerifier().VerifyBytes(Message, signature, publicKey); // throws on failure
    }

    [Fact]
    public void Pinned_signature_rejects_a_tampered_message()
    {
        var publicKey = PqSigningPublicKey.Import(Convert.FromBase64String(PublicKeyVector));
        byte[] signature = Convert.FromBase64String(SignatureVector);
        byte[] tampered = (byte[])Message.Clone();
        tampered[0] ^= 0x01;

        Assert.Throws<PqSignatureException>(() =>
            new PqVerifier().VerifyBytes(tampered, signature, publicKey));
    }

    [Theory]
    [InlineData(HybridSigning.HeaderLength + 5)]                                   // Ed25519 component
    [InlineData(HybridSigning.HeaderLength + SigningSizes.Ed25519Signature + 5)]   // ML-DSA component
    public void Pinned_signature_rejects_tampering_in_either_component(int offset)
    {
        var publicKey = PqSigningPublicKey.Import(Convert.FromBase64String(PublicKeyVector));
        byte[] signature = Convert.FromBase64String(SignatureVector);
        signature[offset] ^= 0x01;

        Assert.Throws<PqSignatureException>(() =>
            new PqVerifier().VerifyBytes(Message, signature, publicKey));
    }
}
